...
def train(resnet50, model, train_loader, test_loader, device, epoch):
    ...
    weights = [0.04, 0.1, 0.03, 0.01, 0.05, 0.01, 0.05, 0.04, 0.19, 0.1, 0.09]
    class_weights = torch.FloatTensor(weights).to(device)
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    max_acc = 0
    max_ep = -1
    with open('task2_loss.csv', 'w', newline='') as csvfile1:
        with open('task2_acc.csv', 'w', newline='') as csvfile2:
            ...
            for ep in range(epoch):
                ...
                for batch_idx, (imgs, labels) in enumerate(train_loader):
                    ...
                    #np.save("task1F_eval/Train-Features-%d" % batch_idx, features.cpu().numpy())
                ...
                if ep % 1 == 0:           
                    valid_loss, acc = task2Test.test(model, test_loader, device)
                    save_checkpoint("task2M/Model-%d.pth" % ep, model, optimizer)
                    if acc >= max_acc:
                        max_acc = acc  
                        max_ep = ep
...
def main():
    ...
    #loader
    print('doading datas')
    trainset = task2Loader.TRIMMED('hw4_data/TrimmedVideos/video/train', 'hw4_data/TrimmedVideos/label/gt_train.csv', transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.485, 0.456, 0.406),(0.229,0.224,0.225))]))
    ...
    train(resnet50, model, train_loader, test_loader, device, 500)
