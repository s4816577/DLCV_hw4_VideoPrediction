...
def readShortVideo(video_path, video_category, video_name, downsample_factor=12, rescale_factor=(224,224)):
    '''
    ...
    for frameIdx, frame in enumerate(videogen):
        if frameIdx % downsample_factor == 0:
            frame = skimage.transform.resize(frame, rescale_factor, mode='constant', preserve_range=True, anti_aliasing=True).astype(np.uint8)
            frames.append(frame)
        else:
            continue
